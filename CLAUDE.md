...
8. **APOE allele probe validation + E2 module score** (`20260518_microglia_APOE_per_cell_call.qmd`): audits the four custom APOE probes against the E4/E4 background (per-cell read-count histograms + per-probe summary table) and computes a continuous E2 score from the two T-allele probes (`rs429358-WT-228` + `rs7412-ALT-226:T`) via `AddModuleScore()` on the SpaNorm assay (`nbin = 10, ctrl = 30`); the score lands as `apoe_E2_module1` on `mg@meta.data` and renders as a `FeaturePlot` on the microglia UMAP, split by condition and (via patchwork) by sample. No binary per-cell call, no ROC / pROC, no genotype-on-cell DE in this notebook.
...
Ingest, QC, four normalization notebooks, the clean SpaNorm re-fit, the microglia sub-cluster build, the microglia pseudobulk DE, and the APOE probe-validation + E2 module-score notebook are all committed. Pseudobulk E4-vs-E2 DE runs on the microglia sub-cluster at Louvain 0.5 (clusters 0 DAM Stage-1, 2 homeostatic canonical, 4 DAM Stage-2, 6 homeostatic low-FC, 10 IRM + proliferating) per state and pooled; only cluster 10 returns FDR < 0.1 hits (Ccl12, Lgals3bp, Mki67 down, Ifit2). The APOE notebook audits the four panel probes against the E4/E4 background and computes a continuous E2 `AddModuleScore` from the two T-allele probes, rendered on the microglia UMAP; no binary per-cell call or genotype-on-cell DE is built. Next steps: build a per-cell E2 / E4 call from the module score (or a ROC-tuned logistic combination of the two E2 probes) and feed it into a genotype-on-cell DE; sub-cluster the consolidated DAM at higher Louvain to recover Stage-1 vs Stage-2 (cluster 4 antigen-presenting Spp1 / Gpnmb / H2-Eb1 / Cd74 vs cluster 0 lipid-chemokine Lyz2 / Cst7 / Ccl3 / Lpl); spatially inspect the IRM cluster near amyloid plaques; sub-cluster the SpaNorm astrocyte pair (clean-0.7 clusters 2, 8) for an IFN-responsive DAA signature. Sample `4s2M_F2` is a partial section; flag it in cluster-composition comparisons.
